...
localnet-start: localnet-stop
	#@if ! [ -f build/node0/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/tendermint:Z tendermint/localnode testnet --v 6 --c 2 . --populate-persistent-peers --starting-ip-address 192.167.10.2 ; fi
	docker-compose up
